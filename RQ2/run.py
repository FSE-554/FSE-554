...
# Global variables
MODELS: Dict[str, str] = {
    # Placeholder for model paths; to be populated later
}

COMBINED_DATASET: str  # Will be populated from arguments
BASE_OUTPUT_DIR: str   # Will be populated from arguments
...
def read_combined_all(path: str) -> List[Dict]:
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if not isinstance(data, list):
        raise ValueError("combined.json must be a list of objects")
...
def extract_code_from_input(input_field: str) -> Optional[str]:
    """
    优先从 ```c ... ``` 代码块中提取；否则将整个 input 视为 C 代码。
    """
...
def smoke_test_vllm(model_source: str) -> bool:
    try:
        from vllm import LLM, SamplingParams
        import torch
        logger.info(f"[SMOKE TEST] torch.cuda.is_available(): {torch.cuda.is_available()}")
        if torch.cuda.is_available():
            logger.info(f"[SMOKE TEST] CUDA: {torch.version.cuda}, GPU count: {torch.cuda.device_count()}")
            logger.info(f"[SMOKE TEST] GPU 0: {torch.cuda.get_device_name(0)}")
        ...
        outs = llm.generate(["hello"], SamplingParams(max_tokens=4, temperature=0.2))
        logger.info(f"[SMOKE TEST] OK: {outs[0].outputs[0].text.strip()!r}")
        return True
    except Exception:
        logger.error(f"[SMOKE TEST] Failed for model: {model_source}", exc_info=True)
        return False
...
def main():
    ...
    logger.info(f"Ray cluster_resources: {ray.cluster_resources()}")
    logger.info(f"Ray available_resources: {ray.available_resources()}")

    print_cuda_info()

    logger.info(f"Loading full dataset from {COMBINED_DATASET}")
    data = read_combined_all(COMBINED_DATASET)
    logger.info(f"Loaded {len(data)} rows from combined.json (full dataset)")
    ...
    for model_key, model_source in MODELS.items():
        ...
        if not smoke_test_vllm(model_source):
            logger.error(f"Smoke test failed for [{model_key}]. Skipping this model.")
            continue

        try:
            results_list = run_with_vllm_direct(data, model_source)

            single_out_dir = os.path.join(BASE_OUTPUT_DIR, model_key)
            os.makedirs(single_out_dir, exist_ok=True)
            single_out_path = os.path.join(single_out_dir, "combined.json")

            with open(single_out_path, "w", encoding="utf-8") as f:
                json.dump(results_list, f, ensure_ascii=False, indent=2)
            logger.info(f"[{model_key}] Wrote single JSON to {single_out_path} (records={len(results_list)})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Multi-model batch inference script for vLLM")
    parser.add_argument("--combined_dataset", type=str, required=True, help="Path to the combined dataset JSON file")
    parser.add_argument("--base_output_dir", type=str, required=True, help="Base output directory for results")
